...
## Mandatory Dev Docs Prompts

- Before any work, you MUST ask the user to run `/dev-docs` (or confirm it already exists) and explicitly wait for confirmation. Do not proceed until the user confirms.
- At the end, you MUST include a concise summary, then ask the user to run `/dev-docs-update` and explicitly wait for confirmation. Do not end the run until the user confirms.
- If the user declines, stop and ask again; do not proceed without confirmation.

## Dev Docs Usage

- After the user confirms `/dev-docs`, you MUST ask which dev docs folder to use (task name) and wait for the answer.
- Then read the plan/context/tasks files from that folder before proceeding.
- If no dev docs exist yet, ask the user to run `/dev-docs` and confirm the folder path.
